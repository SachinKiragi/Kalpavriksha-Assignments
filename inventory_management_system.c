...
void getProductId(productInfo*, productInfo*, int); 
int length(char*);
void getProductName(productInfo*);
void getProductPrice(productInfo*);
void getProductQuantity(productInfo*);
void getCurrentProductInfo(productInfo*, int, int);
...
// Return type void is used instead of bool because the function will not return until the variable 'valid' becomes true
void getProductId(productInfo* currentProduct, productInfo* products, int noOfProducts){
    bool valid = false;
    int id = -1;
    ...
    while(!valid){
        ...
        } else if(doesProductIdExist(id, products, noOfProducts)){
...
void getProductName(productInfo* currentProduct){

    bool valid = false;
    while(!valid){
        printf("Product Name: ");
        fgets(currentProduct->name, MAX_NAME_LENGTH, stdin);
        int len = length(currentProduct->name);
        currentProduct->name[len] = 0;
        if(length(currentProduct->name) > 0){
            valid = true;
        } else{
...
void getProductPrice(productInfo* currentProduct){
    bool valid = false;
    float price = -1;
...
void getProductQuantity(productInfo* currentProduct){
    bool valid = false;
    int quantity = -1;
...
void getCurrentProductInfo(productInfo* products, int noOfProducts, int currentProductIndex){
    getProductId(&products[currentProductIndex], products, noOfProducts);
    getProductName(&products[currentProductIndex]);
    getProductPrice(&products[currentProductIndex]);
    getProductQuantity(&products[currentProductIndex]);
} 
...
void addNewProduct(productInfo** products, int *noOfProducts, int *capacity){
    ...
    getCurrentProductInfo(*products, *noOfProducts, indexToInsertNewProduct);
    (*noOfProducts)++;
...
void swap(productInfo *productA, productInfo *productB){
    productInfo temp = *productA;
    *productA = *productB;
    *productB = temp;
}
...
void handleInventoryManagementSystem(int noOfProducts){
    ...
    for(int i = 0; i < noOfProducts; i++){
        ...
        getCurrentProductInfo(products, noOfProducts, i);
    }
    int choice = 0;
